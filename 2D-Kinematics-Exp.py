...
def prep_frame_number():
    ...
    frame_buffer.prep_frame_num(frm_num)
...
def prev_frame():
    frame_buffer.prep_prev_frame()


def next_frame():
    frame_buffer.prep_next_frame()


def remove_ball_marker():
    frame_buffer.rm_ball_frame_loc()
    prep_frame_number()


def build_and_show_composite():
    # frame_buffer.create_composite_image()
    frame_buffer.prep_frame_num(-1)
...
def change_resolution():
    ...
    frame_buffer.set_display_resolution(resolution=res)
    prep_frame_number()


def set_ref_len_meters():
    len_m = 0.01 * st.session_state["ref_length"]
    frame_buffer.set_ref_len_meters(len_m)
    frame_buffer.calc_m_per_pxl()
...
def tst_img_coords_slctd():
    ...
    if st.session_state["curr_activity"] == "Set Ball Location":
        ...
        frame_buffer.set_plum_line_marker(curr_coords)
        st.session_state["img_coords_prev"] = curr_coords
        frame_buffer.calc_frame_theta()
        prep_frame_number()
        st.experimental_rerun()
...
def panel_mjr_actions():
    ...
    with cols[1]:
        ...
        if frame_buffer.m_per_pxl in [0, 0.0, None]:
            m_per_pxl = "undefined"
            pxl_per_m = "undefined"
        else:
            m_per_pxl = f"{frame_buffer.m_per_pxl:.5f} m/pxl"
            pxl_per_m = f"{1./frame_buffer.m_per_pxl:.2f} pxl/m"
    ...
    with cols[2]:
        ...
        if frame_buffer.image_theta is None:
            theta = "undefined"
        else:
            theta_val = frame_buffer.image_theta * 180 / np.pi
    ...
    with cols[3]:
        st.button(
            ...
            on_click=frame_buffer.set_frame_as_bkgrnd,
        )
        st.write(f"Frame to use as background: {frame_buffer.bkgrnd_frm_num}")
...
frame_buffer = st.session_state["vid_fbf"]

st.title("2D Kinematics Experiment")

if frame_buffer.file_name is None:
    st.file_uploader(
        label="Choose video:",
        type=["mp4", "mov"],
